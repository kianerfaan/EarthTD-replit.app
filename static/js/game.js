class Game {
    constructor() {
        ...
        // Multiple paths for aliens remain unchanged
        this.paths = {
            alaska: [
        ...
        // Store canvas scale for coordinate conversion
    ...
    handlePlacement(coords) {
        if (this.ui.selectedTower && this.canPlaceTower(coords.gridX, coords.gridY)) {
            this.placeTower(coords.gridX, coords.gridY);
        }
    }
    ...
    gameLoop() {
        ...
        if (this.isWaveActive && this.aliens.length === 0) {
            this.wave++;
            this.isWaveActive = false;
            this.gold += 50 + Math.floor(this.wave * 5);
            this.ui.updateResources();
        }
